...
class PedirTurnoIngElectronicaPage extends StatefulWidget{

  PedirTurnoIngElectronicaPage({this.auth, this.onSignedOut});
  final BaseAuth auth;
  final VoidCallback onSignedOut;
  @override

  
    State<StatefulWidget> createState () => new _PedirTurnoIngElectronicaPage();
   

}
class _PedirTurnoIngElectronicaPage extends State<PedirTurnoIngElectronicaPage>{
  ...
  void crearTurno() async {
    ...
    getTurno(String emailsito) {
      return db
          .collection('TurnosIngElectronica')
    ...
    getTurno(emailUsu).then((QuerySnapshot docs) {
      if (docs.documents.isEmpty) {
        ...
        Buscar().buscarusuario(user.email).then((QuerySnapshot docs) async {
          ...
          var refe2 = db.collection('TurnosIngElectronica').document('--turnos--');
          batch.setData(
              refe2,
              {
          ...
          var document =await Firestore.instance.document('TurnosIngElectronica/--turnos--');
          ...
          var refe = db.collection('TurnosIngElectronica').document('$lol');
          var refeEstadistica = db.collection('TurnosIngElectronica_Estadistica').document('$lol');
          ...
          DocumentReference ref2 = await db
              .collection('TurnosIngElectronica_Tokens')
