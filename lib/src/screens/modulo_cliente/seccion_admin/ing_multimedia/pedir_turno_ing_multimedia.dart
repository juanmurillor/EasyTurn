...
class PedirTurnoIngMultimediaPage extends StatefulWidget{

  PedirTurnoIngMultimediaPage({this.auth, this.onSignedOut});
  final BaseAuth auth;
  final VoidCallback onSignedOut;
  @override

  
    State<StatefulWidget> createState () => new _PedirTurnoIngMultimediaPage();
   

}
class _PedirTurnoIngMultimediaPage extends State<PedirTurnoIngMultimediaPage>{
    ...
    getTurno(emailUsu).then((QuerySnapshot docs) {
      if (docs.documents.isEmpty) {
        ...
        Buscar().buscarusuario(user.email).then((QuerySnapshot docs) async {
          ...
          var refe2 = db.collection('TurnosIngMultimedia').document('--turnos--');
          batch.setData(
              refe2,
              {
          ...
          var document =await Firestore.instance.document('TurnosIngMultimedia/--turnos--');
          ...
          var refe = db.collection('TurnosIngMultimedia').document('$lol');
          var refeEstadistica = db.collection('TurnosIngMultimedia_Estadistica').document('$lol');
          ...
          DocumentReference ref2 = await db
              .collection('TurnosIngMultimedia_Tokens')
