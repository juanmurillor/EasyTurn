...
class PedirTurnoIngAgroPage extends StatefulWidget{

  PedirTurnoIngAgroPage({this.auth, this.onSignedOut});
  final BaseAuth auth;
  final VoidCallback onSignedOut;
  @override

  
    State<StatefulWidget> createState () => new _PedirTurnoIngAgroPage();
   

}
class _PedirTurnoIngAgroPage extends State<PedirTurnoIngAgroPage>{
    ...
    getTurno(emailUsu).then((QuerySnapshot docs) {
      if (docs.documents.isEmpty) {
        ...
        Buscar().buscarusuario(user.email).then((QuerySnapshot docs) async {
          ...
          print("este es el nombre " + resultado[0][0]);
          ...
          var refe2 = db.collection('TurnosIngAgro').document('--turnos--');
          batch.setData(
              refe2,
              {
          ...
          var document =await Firestore.instance.document('TurnosIngAgro/--turnos--');
          ...
          var refe = db.collection('TurnosIngAgro').document('$lol');
          var refeEstadistica = db.collection('TurnosIngAgro_Estadistica').document('$lol');
          ...
          DocumentReference ref2 = await db
              .collection('TurnosIngAgro_Tokens')
